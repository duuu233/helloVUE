...
service.interceptors.request.use(config => {
  ...
  if (!tokenTime) {
    ...
    if (newTime - tokenTime >= 86400000 && config.url.indexOf('login') === -1) {
  ...
  //签名-------------s
  let randomString = new Date().toLocaleDateString().split("/").join("") +
  ...
  if (config.method === 'get') {
    ...
    if (getToken()) {
      config.params.adminToken = getToken();
    }
  }
  if (config.method === 'post') {
    ...
    if (getToken()) {
      config.data.adminToken = getToken();
    }
    // 过滤null
