      ...
      <n-gi v-bind="schema.giProps" v-for="schema in getSchema" :key="schema.field">
        <n-form-item :label="schema.label" :path="schema.field">
          ...
          <template v-else-if="schema.component === 'NSwitch'">
            <n-switch v-model:value="formModel[schema.field]">
              <template #checked>
                启用
              </template>
              <template #unchecked>
      ...
      <n-gi :span="isInline ? '' : 24" :suffix="isInline ? true : false" #="{ overflow }"
        ...
        <n-space align="center" :justify="isInline ? 'end' : 'start'"
          ...
          <n-button v-if="getProps.showSubmitButton" v-bind="getSubmitBtnOptions" @click="handleSubmit"
            :loading="loadingSub">{{ getProps.submitButtonText }}</n-button>
...
import { BasicUploadNk } from '@/components/Upload-nk';
...
export default defineComponent({
  name: 'BasicUpload',
  components: { DownOutlined, UpOutlined, QuestionCircleOutlined, BasicUploadNk },
  props: {
    ...basicProps,
  },
